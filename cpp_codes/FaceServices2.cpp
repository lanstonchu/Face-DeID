...
FaceServices2::FaceServices2(const std::string & model_file) // constructor
{
	...
	im_render = nullptr;
        printf("load %s\n",model_file.c_str());
	festimator.load3DMM(model_file); //festimator is a BaselFaceEstimator object defined in header; model_file can be /3DMM_model/BaselFaceModel_mod.h5
}

FaceServices2::~FaceServices2(void) //Destructor
{
}

// Setup with image size (w,h) and focal length f
void FaceServices2::init(int w, int h, float f){ // e.g. (500,500,1000.0)
	// Instrinsic matrix
	memset(_k,0,9*sizeof(float)); // i.e. memory setting; to assign value of zeros to _k, which is 9 floats
	_k[8] = 1;
	_k[0] = -f;
	_k[4] = f;
	_k[2] = w/2.0f;
...
// Lanston's functions -- START

// copied from main.cpp
// Load shape parameters from file (99x1)
cv::Mat loadWeightFS2(const char* inputFile){ // FS: FaceService
	ifstream in_stream(inputFile);
	if (!in_stream.is_open()) {
		printf(".alpha file doesn't exist.\n");
		exit(1); // modified by Lanston to ensure .alpha files exist
		// return cv::Mat();
	}
...
// Lanston's functions -- END

// Estimate pose and expression from landmarks // original line number: 53
//     Inputs:
//	   colorIm    : Input image
...
bool FaceServices2::estimatePoseExpr(cv::Mat colorIm, cv::Mat lms, cv::Mat alpha, cv::Mat &vecR, cv::Mat &vecT, cv::Mat &exprW){
	char text[200];
	float renderParams[RENDER_PARAMS_COUNT]; // RENDER_PARAMS_COUNT is 21, see line 6 of RenderModel.h
	float renderParams2[RENDER_PARAMS_COUNT];
	Mat k_m(3,3,CV_32F,_k); // load the intrinsic matrix k_m
	BFMParams params; // see FaceServices2.h:21
	params.init();
	...
	// get 3D landmarks
	Mat landModel0 = festimator.getLM(shape,0); // 0 is refers to the yaw angle
	...
	for (int i=0;i<3;i++)
		params.initR[RENDER_PARAMS_R+i] = vecR.at<float>(i,0); // 0:2 is for rotation; RENDER_PARAMS_R = 0, see RenderModel.h
	for (int i=0;i<3;i++)
		params.initR[RENDER_PARAMS_T+i] = vecT.at<float>(i,0); // 3:5 is for translation; RENDER_PARAMS_T = 3, see RenderModel.h
	...
	int iter=0;
	int badCount = 0;
	memset(params.doOptimize,true,sizeof(bool)*6); // first 6 doOptimize-flags (i.e. rotation/translation) is 1

	// optimize pose+expression from landmarks
	int EM = 29; // exprW is dim(29,1)
	...
	memset(params.doOptimize,false,sizeof(bool)*6);countFail = 0; // first 6 doOptimize-flags (i.e. rotation/translation) is 0
	for (;iter<200;iter++) {
			if (iter%60 == 0) {
				cCost = updateHessianMatrix(alpha,renderParams,faces,colorIm,lmVisInd,landIm,params, exprW);
				if (countFail > 10) {
...
cv::Mat FaceServices2::renderShape(cv::Mat colorIm, cv::Mat alpha,cv::Mat vecR,cv::Mat vecT,cv::Mat exprW){
	float renderParams[RENDER_PARAMS_COUNT]; // renderParams: r (3), t(3), AMBIENT (3), DIFFUSE (3), LDIR (2), CONTRAST (1), GAIN (3), OFFSET (3), ...
	for (int i =0;i<3;i++)
		renderParams[i] = vecR.at<float>(i,0); // copy R of size-3
	for (int i =0;i<3;i++)
		renderParams[i+3] = vecT.at<float>(i,0); // copy T of size-3

	// Ambient
	renderParams[RENDER_PARAMS_AMBIENT] = 0.69225; // value of RENDER_PARAMS_AMBIENT is in RenderModel.h
	...
	// render
	cv::Mat outRGB = cv::Mat::zeros(colorIm.rows,colorIm.cols,CV_8UC3); // i.e. size of original image
	cv::Mat outDepth = cv::Mat::zeros(colorIm.rows,colorIm.cols,CV_32F); // i.e. size of original image
...
// Compute Hessian matrix diagonal; output is params.hessDiag, will only compute 199:(199 + 29 + 6), i.e. 2nd-derivatives for expression/rotation/translation only
float FaceServices2::updateHessianMatrix(cv::Mat alpha, float* renderParams, cv::Mat faces, cv::Mat colorIm,std::vector<int> lmInds, cv::Mat landIm, BFMParams &params, cv::Mat exprW ){
	...
	int EM = exprW.rows;
	float step;
	cv::Mat k_m( 3, 3, CV_32F, _k ); // load the intrinsic matrix k_m
	cv::Mat distCoef = cv::Mat::zeros( 1, 4, CV_32F );
	params.hessDiag.release(); // BFMParams (class of params) defined in line 21 of FaceServices2.h
	params.hessDiag = cv::Mat::zeros(2*M+EM+RENDER_PARAMS_COUNT,1,CV_32F); // hessDiag: dim(2*99+29+21, 1), i.e. there are 248 parameters to be trained
	cv::Mat alpha2, expr2;
	float renderParams2[RENDER_PARAMS_COUNT];

	cv::Mat rVec(3,1,CV_32F,renderParams+RENDER_PARAMS_R); // the 4th entry is the pointer address
	...
	float currEF = eF(alpha, lmInds, landIm, renderParams, exprW); // Compute landmark error, see line 449
	cEF = currEF;

	// expr
	step = mstep*5; // i.e. 0.0001*5
	if (params.optimizeExpr) { // i.e. to approximate \partial^2_expression eF() (i.e. landmark error)
		for (int i=0;i<EM; i++){ // running over 1 to 29
			expr2.release(); expr2 = exprW.clone(); // exprW is dim(29,1)
			expr2.at<float>(i,0) += step; // i.e. plus-step
			float tmpEF1 = eF(alpha, lmInds, landIm, renderParams,expr2);
			expr2.at<float>(i,0) -= 2*step; // i.e. minus-step; minus 2 steps to offset the plus-step
			float tmpEF2 = eF(alpha, lmInds, landIm, renderParams,expr2);
			params.hessDiag.at<float>(2*M+i,0) = params.weightLM * (tmpEF1 - 2*currEF + tmpEF2)/(step*step) // approximation of the 2nd-partial-derivatives;
				+ params.weightRegExpr * 2/(0.25f*29) ; // weightRegExpr: weights for expression parameter regularization
		}
	}
	// r
	step = mstep*2;
	if (params.doOptimize[RENDER_PARAMS_R]) { // i.e. to approximate \partial^2_rotation eF() (i.e. landmark error)
		for (int i=0;i<3; i++){
	...
	// t
	step = mstep*10;
	if (params.doOptimize[RENDER_PARAMS_T]) { // i.e. to approximate \partial^2_translation eF() (i.e. landmark error)
		for (int i=0;i<3; i++){
			memcpy(renderParams2,renderParams,RENDER_PARAMS_COUNT*sizeof(float));
...
// Compute gradient, i.e. partial derivatives on eF()
...
// Compute landmark error between landIm and f(the rest)
float FaceServices2::eF(cv::Mat alpha, std::vector<int> inds, cv::Mat landIm, float* renderParams, cv::Mat exprW){
	Mat k_m(3,3,CV_32F,_k); // load the intrinsic matrix k_m
	cv::Mat mLM = festimator.getLMByAlpha(alpha,-renderParams[RENDER_PARAMS_R+1], inds, exprW); // mLM is the 3D landmarks, i.e. size: 68x3
	...
	cv::projectPoints( mLM, rVec, tVec, k_m, distCoef, allImgPts ); // Projects 3D points to an image plane. allImgPts is output
	float err = 0;
	for (int i=0;i<mLM.rows;i++){ // i going over number of landmarks; computing ||landIm - allImgPts||^2
...
// Newton optimization step; output: exprW in dim(29,1) and rotation/translation in dim(6,1) (i.e. first 6 in the render parameters) such that error = 0
void FaceServices2::sno_step(cv::Mat &alpha, float* renderParams, cv::Mat faces,cv::Mat colorIm, std::vector<int> lmInds, cv::Mat landIm, BFMParams &params, cv::Mat &exprW){
	...
	if (params.optimizeExpr){
		for (int i=0;i<EM;i++)
			if (abs(params.hessDiag.at<float>(2*M+i,0)) > 0.0000001) {
				dirMove.at<float>(2*M+i,0) = - lambda*dE.at<float>(2*M+i,0)/abs(params.hessDiag.at<float>(2*M+i,0)); // i.e. Newtown optimization: x = x - f'/f''
			}
	}

	for (int i=0;i<RENDER_PARAMS_COUNT;i++) {
		if (params.doOptimize[i]){ // only do update for the first 6 parameters (i.e. rotation and translation)
	...
	float pc = line_search(alpha, renderParams, dirMove,inds, faces, colorIm, lmInds, landIm, params, exprW, 10); //find best exprW2 and render parameters to maximize cost
	if (pc == 0) countFail++; // i.e. exceeding maximum iteration but still can't find best exprW/render parameters
	else {
		if (params.optimizeExpr){
			for (int i=0;i<EM;i++) {
				exprW.at<float>(i,0) += pc*dirMove.at<float>(i+2*M,0); // update exprW
...
// Line search optimization; output: step; given the searching direction dirMove, output is the step size such that we can update exprW2 and render such that cost is maximized; but we don't really update in this function.
float FaceServices2::line_search(cv::Mat &alpha, float* renderParams, cv::Mat &dirMove,std::vector<int> inds, cv::Mat faces,cv::Mat colorIm,std::vector<int> lmInds, cv::Mat landIm, BFMParams &params, cv::Mat &exprW, int maxIters){
	...
	alpha2 = alpha.clone();
	exprW2 = exprW.clone();
	memcpy(renderParams2,renderParams,sizeof(float)*RENDER_PARAMS_COUNT); // copy from renderParams to renderParams2
	...
	int M = alpha.rows; // i.d. 99
	int EM = exprW.rows; // i.d. 29
	float ssize = 0;
	for (int i=0;i<dirMove.rows;i++) ssize += dirMove.at<float>(i,0)*dirMove.at<float>(i,0); // i.e. ||dirMove||^2_2
	ssize = sqrt(ssize); // i.e. ssize = ||dirMove||_2
	...
	for (; iter<maxIters; iter++){
		if (params.optimizeExpr){ // update expression
			for (int i=0;i<EM;i++) {
				float tmp = exprW.at<float>(i,0) + step*dirMove.at<float>(2*M+i,0); // updated exprW?
				if (tmp >= 3) exprW2.at<float>(i,0) = 3; // to cap exprW
				else if (tmp <= -3) exprW2.at<float>(i,0) = -3; // to floor exprW
				else {
		...
		for (int i=0;i<RENDER_PARAMS_COUNT;i++) {
			if (params.doOptimize[i]){ // update render parameters
		...
		float tmpEF = cEF; // just to copy the size of cEF?
		...
		if (tmpCost < curCost) {
			break; // stop if the cost is smaller than the current cost. that mean we are maximizing the cost, but why?
		}
		else {
			step = step/sstep; // reduce the step by half if we don't stop
		}
	}
...
float FaceServices2::computeCost(float vEF, cv::Mat &alpha, float* renderParams, BFMParams &params, cv::Mat &exprW ){
	...
	int M = alpha.rows; // dim-99
	if (params.optimizeExpr){ // optimizeExpr: flag indicating whether expression is optimizing
		for (int i=0;i<exprW.rows;i++)
			val += params.weightRegExpr * exprW.at<float>(i,0)*exprW.at<float>(i,0)/(0.5f*29); //weightRegExpr is for regularization
	}

	for (int i=0;i<RENDER_PARAMS_COUNT;i++) {
		if (params.doOptimize[i]){ // doOptimize flags indicating which rendering parameters is optimizing
...
// combineBump is a method of the class FaceServices2 as per FaceServices2.h
bool FaceServices2::combineBump(cv::Mat colorIm, cv::Mat lms, cv::Mat alpha, cv::Mat bumpMap, cv::Mat maskMap, string out_prefix, OutputSettings outSet, bool softSym, int margin){ // the end is at line 1800
	char text[200];
	float renderParams[RENDER_PARAMS_COUNT];
	float renderParams2[RENDER_PARAMS_COUNT];
	Mat k_m(3,3,CV_32F,_k); // load the intrinsic matrix k_m. CV_32F is a CVtype, which is 5 and refers to bit-depth. It is 1 channel of 32 bits float. _k is defined in FaceServices2::combineBump
	BFMParams params; // BFMParams defined in line 21 of FaceServices2.h
	params.init();
	cv::Mat vecR, vecT;
	cv::Mat exprW = cv::Mat::zeros(29,1,CV_32F); // exprW shold refers to expression which is dim-29
	std::vector<int> pairInd = festimator.getPair(); // pairInd is dim-46990; festimator is a BaselFaceEstimator object, as per line 101 of FaceServices2.h. festimator is imported from "/3DMM_model/"BaselFaceModel_mod.h5" at the constructor

	int M = 99;
	// get subject shape
	cv::Mat shape = festimator.getShape(alpha); // alpha is dim-198. shape is dim-(46990,3); TODO: don't understand why exprW can be omitted...

	// get 3D landmarks
	Mat landModel0 = festimator.getLM(shape,0); // 3D-landmarks. landModel0 is dim-68
	int nLM = landModel0.rows;

	// compute 3D pose w/ the first 60 (i.e. 61 to 68 are inside-month) 2D-3D correspondences
	Mat landIm = cv::Mat( 60,2,CV_32F);
	Mat landModel = cv::Mat( 60,3,CV_32F);
	for (int i=0;i<60;i++){ // only take the first 60 landmarks (i.e. outside-month)
		landModel.at<float>(i,0) = landModel0.at<float>(i,0); // landModel0 refers to 3D landmarks
		landModel.at<float>(i,1) = landModel0.at<float>(i,1);
		landModel.at<float>(i,2) = landModel0.at<float>(i,2);
		landIm.at<float>(i,0) = lms.at<float>(i,0); // lms refers to 2D landmarks
		landIm.at<float>(i,1) = lms.at<float>(i,1);
	}
	festimator.estimatePose3D(landModel,landIm,k_m,vecR,vecT); // to determine vecR and vecT (i.e. rotation and translation; both by-reference) of image; value of k_m is defined in FaceServices2::FaceServices2;

	// reselect 3D landmarks given estimated yaw angle
	float yaw = -vecR.at<float>(1,0); // not accurate concept, because the rotation is along vecR, and R_x*R_y not equal R_y*R_x
	landModel0 = festimator.getLM(shape,yaw); // is the yaw angle is big, another set of landmarks will be selected here
	// select landmarks to use based on estimated yaw angle
	std::vector<int> lmVisInd; // lmVisInd seems to be "indicator of the visible landmarks"
	for (int i=0;i<60;i++){
		if (i > 16 || abs(yaw) <= M_PI/10 || (yaw > M_PI/10 && i > 7) || (yaw < -M_PI/10 && i < 9))
			lmVisInd.push_back(i); // selected i (probably because it's shown at the yaw angle) will be included in the lmVisInd array
	}
	landModel = cv::Mat( lmVisInd.size(),3,CV_32F);
	landIm = cv::Mat::zeros( lmVisInd.size(),2,CV_32F);
	for (int i=0;i<lmVisInd.size();i++){
	...
	// resetimate 3D pose
	festimator.estimatePose3D(landModel,landIm,k_m,vecR,vecT); // to determine vecR and vecT again based on the visible landmarks

	for (int i=0;i<3;i++) // store rotation/translation into params
		params.initR[RENDER_PARAMS_R+i] = vecR.at<float>(i,0);
	for (int i=0;i<3;i++)
		params.initR[RENDER_PARAMS_T+i] = vecT.at<float>(i,0);
	memcpy(renderParams,params.initR,sizeof(float)*RENDER_PARAMS_COUNT); // copy blocks of memory from params.initR to renderParams
	...
	int iter=0;
	int badCount = 0;
	memset(params.doOptimize,true,sizeof(bool)*6); // i.e. only do error/cost-optimization (e.g. Hessian computation) for the first 6 flags (i.e. rotation/translation) for params; i.e. flat set as 1

	// optimize pose+expression from landmarks
	int EM = 29; // size of expression-embedding
	float renderParams_tmp[RENDER_PARAMS_COUNT];

	for (;iter<60;iter++) { // first 60 iterations: optimize both exprW and rotation/translation
			if (iter%20 == 0) { // update Hessian every 20 iterations
				cCost = updateHessianMatrix(alpha,renderParams,faces,colorIm,lmVisInd,landIm,params, exprW); // see line 321. params.hessDiag will be updated as \partial^2_{expression, rotation, translation}
				if (countFail > 10) {
					countFail = 0;
					break;
				}
			}
			sno_step(alpha, renderParams, faces,colorIm,lmVisInd,landIm,params,exprW); // determine exprW (29) and rotation/translation (6) by using alpha and image
		}
	iter = 60;


	memset(params.doOptimize,false,sizeof(bool)*6);countFail = 0;
	for (;iter<200;iter++) { // for iteration 61 to 200, only optimize expression but fixing rotation/translation
			if (iter%60 == 0) {
				cCost = updateHessianMatrix(alpha,renderParams,faces,colorIm,lmVisInd,landIm,params, exprW);
				if (countFail > 10) {
					countFail = 0;
		...
		// Lanston's code: start; replace alpha after determining expression
		printf("\n\n----Lanston's code: START----\n");
    char myStr[200];
		strcpy(myStr,out_prefix.c_str()); // e.g. /shared/output/output_X/3D//150-11 for the 150th person
		printf("%s\n", myStr);

		char *file_prefix = strstr(myStr, "//") + 2; // file prefix without directory, e.g. 150-11
		char *person_i = strtok(file_prefix, "-"); // e.g. 150

		char *rightPartial1 = strstr(myStr, "/") + 1;
		char *rightPartial2 = strstr(rightPartial1, "/") + 1;
		char *rightPartial3 = strstr(rightPartial2, "/") + 1;
		char *rightPartial4 = strstr(rightPartial3, "/") + 1;

		char ns_flag = rightPartial3[11];
		char *middlePartial4 = strtok(rightPartial4, "/"); // pbX_pivotY
		char *pivot_i = strstr(middlePartial4, "_") + 1; // pivot20 or pivot100
		char *pb_i = strtok(middlePartial4, "_"); // pb2 or pb10

		char ns_text_i[10]; // normal or smile
		if (ns_flag=='n'){
			strcpy(ns_text_i,"normal");
		}else if (ns_flag=='s'){
			strcpy(ns_text_i,"smile");
		}else{
			fprintf(stderr, "ns_flag should be either n or s.\n");
			exit(1);
		}

		// form the full path of .alpha
		char perturbed_alpha_text[200];
		sprintf(perturbed_alpha_text, "/app/demoCode/alphas/FEI/person");
		// sprintf(perturbed_alpha_text, "/app/demoCode/alphas/5_old_people/person");
		strcat(perturbed_alpha_text, person_i);
		strcat(perturbed_alpha_text, "_");
		strcat(perturbed_alpha_text, pb_i); // pb2 or pb10
		strcat(perturbed_alpha_text, "_");
		strcat(perturbed_alpha_text, pivot_i); // pivot20 or pivot100
		strcat(perturbed_alpha_text, "_");
		strcat(perturbed_alpha_text, ns_text_i); // normal or smile
		strcat(perturbed_alpha_text, ".alpha");

		printf("%s\n", perturbed_alpha_text);

		alpha = loadWeightFS2(perturbed_alpha_text); // hide/unhide for non-perturbed/perturbed
		printf("----Lanston's code: END----\n\n");
		// Lanston's code: end

	shape = festimator.getShape(alpha,exprW); // original line number: 734
	cv::Mat tex = festimator.getTexture(alpha*0);

	// Foundation shape (s+e)
	if (outSet.foundationFrontal) {
		sprintf(text,"%s_foundation_frontal.ply",out_prefix.c_str()); // text becomes the filename of the .ply
		write_plyShapeFloat(text,shape,faces); // defined in utility.cpp
	}
	cv::Mat faces_fill = festimator.getFaces_fill() - 1; // triangle connectivity of size Fx3
	int NEI = 1;

	///////////////////////////////////////////////////////////////////////////////
	////// Estimate occlusion map
	if (im_render == 0) im_render = new FImRenderer(cv::Mat::zeros(colorIm.rows,colorIm.cols,CV_8UC3)); // im_render is a pointer as per line 99 of FaceServices2.h
	im_render->loadMesh(shape,tex,faces_fill); // loadMesh() defined in FTModel.cpp, i.e. loading vertices and faces of shape in im_render.face_.mesh_
	cv::Mat refRGB = cv::Mat::zeros(colorIm.rows,colorIm.cols,CV_8UC3); // CV_8UC3 is for 3 channels, i.e. RGB
	cv::Mat refDepth = cv::Mat::zeros(colorIm.rows,colorIm.cols,CV_32F);

	float r[3], t[3];
	for (int i=0;i<3;i++){
		r[i] = vecR.at<float>(i,0); // vecR and vecT are re-estimated at line 678 by festimator.estimatePose3D()
		t[i] = vecT.at<float>(i,0);
	}
	im_render->loadModel(); // im_render is a pointer for a FImRenderer object, and thus "->". to load 3D face model of shape from im_render.face to OpenGL? i.e. to put the face into a "OpenGL frustum"?
	im_render->render(r,t,_k[4],refRGB,refDepth); // output: refRGB, and depth buffer refDepth, which is in [0, 1]
		// Compute occlusion scores (Hassners 2014)
	cv::Mat occMat = computeOccMat(im_render, r, t); // defined in line 1803. here, a sparse mesh is already inside the renderer?
		// Compute visibility based on projection & input segmentation map
	cv::Mat mask = refDepth < 0.9999; // "refDepth < 0.9999" means object exist before z_far, i.e. inside frustum
	cv::Mat maskMap_crop = maskMap(cv::Rect(6,6,mask.cols,mask.rows)); // Rect: (x,y,width, height); cropping the black-white-mask; (6,6) is for margin removal?
	mask = mask & (maskMap_crop > 127); // i.e. "2D mask final" = "2D mask from 3D" & "2D mask from 2D"
	bool* visible = 0;
	...
	cv::Mat matR;
	cv::Rodrigues(vecR,matR); // convert rotation vector vecR into rotation matrix matR
	cv::Mat rotShape = matR * shape.t() + cv::repeat(vecT,1,shape.rows); // A.t() = transpose(A); i.e. all vertices get rotated and then translated; rotShape seems to be the sparse mesh
	rotShape = rotShape.t();
	cv::Mat rotShape0 = rotShape.clone();

	float zNear_ = im_render->zNear; // zNear and zFar are distances away from the "camera" or the eye of the view in world units
	float zFar_ = im_render->zFar;
		// Compute mindepth & prepare vertex indices (vindex) for the dense mesh
	float minDepth = 0;
	std::vector<int> pointInd[2]; // pointInd will contain two 1-vectors, one stores x of points and one stores y of points
	cv::Mat vindex = cv::Mat::zeros(mask.rows,mask.cols,CV_32S)-1;
	int numNeighbors = 0;
	for (int x=0;x<refDepth.cols;x++){ // all vertical pixels in the size of original image
		for (int y=0;y<refDepth.rows;y++){ // all horizontal pixels in the size of original image
                        if (maskMap.rows > 0 && maskMap.at<unsigned char>(y+6,x+6) < 127) continue; // skip codes of the rest if conditions meet
			float dd = refDepth.at<float>(y,x);
			if (dd<0.9999){ // if object exists
				vindex.at<int>(y,x) = pointInd[0].size(); // push the point coordinate (y,x) (with unit = pizel) to vindex if (y,x) exists in "face-projection" (i.e. dd<0.9999), and assign an integer to each (y,x) ascendingly
				pointInd[0].push_back(y);
				pointInd[1].push_back(x);
				refDepth.at<float>(y,x) = - zNear_*zFar_   / ( zFar_ - dd * ( zFar_ - zNear_ )); // replace depth buffer dd by actual depth z
				if (minDepth > refDepth.at<float>(y,x))
					minDepth = refDepth.at<float>(y,x); // finally, minDepth = min(refDepth value of (x,y) in human face mask)
			}
		}
	}
	...
	refDepth = refDepth-minDepth; // starting from here, refDepth belongs to [0,dp_max - dp_min], v belongs to [dp_min, dp_max]
	cv::Mat v(pointInd[0].size(),3,CV_32F); // create v; #(points in v) = #(pixel on which face exists)
	cv::Mat v00 = v.clone();
	for (int i=0;i<pointInd[0].size();i++){
			v.at<float>(i,2) = refDepth.at<float>(pointInd[0][i],pointInd[1][i])+minDepth; // setting the distance back to original depth. NOT adding wrinkle;
			v.at<float>(i,0) = v.at<float>(i,2)*(pointInd[1][i] - _k[2])/_k[0]; // horizontal direction projection adjustment; pushing the human face from focal length distance to original depth
			v.at<float>(i,1) = v.at<float>(i,2)*(pointInd[0][i] - _k[5])/_k[4]; // vertical direction projection adjustment;
	...
	std::vector<Vec3i> fac; // Vec3i: Each entry is 3 integers
	computeFaces(vindex,fac); // defined at line 2006; output: fac; i.e. points --> triangles; vindex is dense in 2D sense with integral entries
	cv::Mat fac2(fac.size(),3,CV_32S); // create fac2
	for (int i=0;i<fac.size();i++){ // copy fac to fac2
	...
	for (int j=0;j<pointInd[0].size();j++){
		int x = pointInd[1][j];
		int y = pointInd[0][j];
		unsigned char change0 = bumpMap.at<unsigned char>(y+6,x+6); // bumpMap is imported from the XXX_bump.png
		float change = change0;
		change = (change-127)/12 * 1.25;
		v.at<float>(j,2) += change; // adding wrinkle; section 4.1 of paper. z(p) + minDepth; the depth is direction-2
		refDepth.at<float>(y,x) = v.at<float>(j,2) - minDepth; // update refDepth with wrinkle as well
	...
	for (int i=0;i<pointInd[0].size();i++){
		...
		int y = pointInd[0][i];
		int x = pointInd[1][i];
		int sx = (x-NEI>=0)?(x-NEI):0; // i.e. sx = x - NEI for most cases; NEI = 1;
		int ex = x<=(refDepth.cols-1-NEI)?(x+NEI):(refDepth.cols-1); // i.e. ex = x + NEI for most cases
		int sy = (y-NEI>=0)?(y-NEI):0;
		int ey = y<=(refDepth.rows-1-NEI)?(y+NEI):(refDepth.rows-1);
		for (int rx=sx;rx<=ex;rx++) // looping over neighbors of x
			for (int ry=sy;ry<=ey;ry++)
				if ((rx != x || ry != y) && mask.at<unsigned char>(ry,rx)>0) // if the neighbors is also on the human face
			{
					numNeighbors++;
	...
	v = matR.t() * v.t() + repeat(-matR.t() * vecT, 1, v.rows) ; // rotation back; compare to line 789; v at L.H.S. is the 3D "undo-projection" of 2D mask based on bumpmap
	cv::Mat best_v = v.t();
...
// Compute triangles from the vertex index grid; i.e. points --> triangles; output: updated_faces
void FaceServices2::computeFaces(cv::Mat findex, vector<cv::Vec3i> &updated_faces){ //seems that: findex are coordinates of dense mesh
	int index[3];
	updated_faces.clear();
	for( int r= 0; r< findex.rows - 1; r += 1 ) { // along vertical pixel
		for( int c= 0; c< findex.cols - 1; c += 1 ) { // along horizontal pixel
			index[0] = findex.at<int>(r,c+1);
			if (index[0] >= 0) {
				index[1] = findex.at<int>(r,c);
				index[2] = findex.at<int>(r+1,c);
				if (index[1] >= 0 && index[2] >= 0)
					updated_faces.push_back(cv::Vec3i(index[0],index[1],index[2])); //Vec3i: vector of dim-3 in integer
				...
				if (findex.at<int>(r,c+1) >= 0 && findex.at<int>(r+1,c) >= 0 && findex.at<int>(r+1,c+1) >= 0)
					updated_faces.push_back(cv::Vec3i(findex.at<int>(r,c+1),findex.at<int>(r+1,c),findex.at<int>(r+1,c+1))); // same as the 2nd updated_faces() ?
		}
	}
}
